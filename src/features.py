...
def ssd(f, g):
    """ Return the sum of squared differences between f and g.
    Pre: f and g have the same shape. """
    return np.sum((g - f)**2)
...
def visualize_harris(img, i, j, topdown=True, window_halfwidth=3, error_surface_halfwidth=6):
    ...
    ax = plt.subplot(2, 2, 1, title="Window around the candidate feature point")
    ax = plt.gca()
    ax.cla()
